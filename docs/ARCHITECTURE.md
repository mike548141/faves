...
validated by `tools/products.py`, with no screen reading them yet, no eating
event permitted in one, and — since ADR 0115 superseded ADR 0090's rule 1 —
no **coordinate**: where a product was seen may be a venue reference or a shop
name, never a number, and reading GPS at intake is explicitly untouched), and
two records **about the intake material itself** (`data/intake/`): the bursts
read and deliberately not a product (`not-products.json`, shape-checked by
`tools/products.py` and reconciled against the photographs by its `--coverage`
mode) and the name index
of the prior food exports ADR 0090 promised (`name-index.json`, generated by
`tools/intake_index.py` — names and source file only, never a nutrition figure
from those exports). The record
...
```jsonc
{
  ...
  "vibe": ["quick-eats", "byo"],    // CLOSED vocabulary — site/js/vibes.js (ADR 0084)
...
- **Export / import** (no key of its own): `personal-data.js` —
  ...
  **every** profile, not just the active one. Settings → "Your data" writes it
  ...
  `blocking` questions and apply errors rather than proceeding.
  `personal-io-ui.js` is the thin half — the file picker and the shared review.
  A second door, the `#xfer=` "transfer to another device" link of Theme 9 v1
  (ADR 0030), came in through the same seam until **2026-08-16**, when the owner
  removed it: it sat between the file backup, which restores a whole device, and
  continual sync, which keeps two devices in step, and did neither job. Gone with
  it: `envelopeFromTransfer`, `share-codec.js`'s `encodeTransfer`/`decodeTransfer`
  and the `#xfer=` fragment, and the receive dialog that was wired on all three
  screens. ADR 0030's import half stands unchanged.
