...
location or eating event permitted in one), and two records **about the intake
material itself** (`data/intake/`): the bursts read and deliberately not a
product (`not-products.json`, shape-checked by `tools/products.py` and
reconciled against the photographs by its `--coverage` mode) and the name index
