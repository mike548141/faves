- [ ] 🔎 **The "not one stub is fetchable" sweep missed at least one venue that
  ...
  **not reliable at the record level** — so the sentence telling future sessions
  ...
  🎯 **What is owed, and it is a decision because it costs real fetches:**
  1. **Re-check all remaining stubs** by name search rather than by the record's
     `website` field, and record *what was searched* alongside the verdict so
     the next refutation is cheap. Most thorough; ~13 venues.
  2. **Re-check only the "publishes nothing at all" group** — the class the
     miss came from — and leave the four known-site-no-menu records alone.
  3. **Leave it**, and amend `150` to say its negative verdict is per-method,
     not per-venue, so nobody reads it as proof again.
  📌 **Whichever is chosen, `150`'s "do not repeat this search" sentence should
  be softened**, because it is currently load-bearing and demonstrably wrong at
  least once.
