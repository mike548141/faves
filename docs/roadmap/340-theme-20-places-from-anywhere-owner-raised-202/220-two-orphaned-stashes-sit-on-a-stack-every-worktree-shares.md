- [x] 🚩 **Two orphaned stashes have sat on the shared stash stack for three
      weeks, and nothing in the house rules says anyone should ever look**
      ✅ **CLOSED 2026-09-21 at merge (branch `stash-residue-check`, ADR
      0119)** — the bracket flipped and the verify-list line landed in
      `CLAUDE.md` by the merging session, as the worker's hand-over below
      asked. `--selftest` re-run at merge: 14/14.
