...
| the distance-limit lines (ADR 0091) | **Your 5 km distance limit is hiding 19 places.** · **Nothing to show within your 5 km distance limit.** · **19 places further away are hidden.** · **📍 14 km away — further than your 5 km distance limit, so it isn't on your home list.** · **Widen to 10 km** · **Change your distance limit** | Carry **no key at all**, and cannot: every one interpolates a live figure (the limit, the count, the distance), and `reo.js` swaps **whole strings only** — a key here would render the English sentence with the numbers of a different one. Listed so the gap is a queue item rather than a silence. 🚩 For the reviewer: these are what a reader meets when their own setting has emptied or shortened the screen, so the tone is **matter-of-fact, never an apology and never a warning** — the setting is theirs and it is working. If these are ever to be translated, `reo.js` needs a parameterised form first; that is a change to the engine, not to this table. |
...
| `nav.shopping`, and the whole shopping list (17e) | **Shopping list** · **Add to shopping list** / **On your shopping list** · **Update the amounts** · **View the list** · **Clear the list** · **Remove** · **Your list holds this recipe at different amounts from the ones shown.** | Added 2026-09-24 with the shopping list ([ADR 0124](decisions/0124-the-shopping-list-is-the-order-tallys-twin-not-a-second-list.md)). Only the first carries a key at all; the rest are composed in JS, and two of them **swap under the reader's own tap** — `reo.js` swaps whole strings only and `translate()` captures the English once, so a half-wired key would pin whichever label happened to be showing. 🚩 For the reviewer: **"shopping list" is the one to settle first**, because every other string here quotes it. 🔎 **Checked, not assumed (2026-09-24):** the owner's nominated source, maoridictionary.co.nz, returns *“We couldn't find that”* for the phrase **shopping list**, and likewise for the obvious composed candidate *rārangi hokohoko*. So there is nothing to copy and the only way to a gloss is to BUILD one from single-word lookups — the same refusal this file already records for *dining*. It wants a speaker's phrase for the whole thing, not a noun plus a modifier. The toggle pair must read as **a state and an action**, and the "different amounts" line is a **fact about the reader's own list, never a warning that something is wrong** — they are allowed to shop at 1× while reading 2×. |
