# Faves session log — archive

Verbatim older entries rotated out of [`SESSIONS.md`](SESSIONS.md) on
2026-07-18 to keep the always-loaded session index lean. Entries keep their
original order (newest last) and their exact text — rotation moves them here
untouched, never edits them (append-only content: the current-truth/history
split, same as `ROADMAP`→`ROADMAP-DONE`). Grep this file when the recent
tail isn't enough; do not load it whole into context.
