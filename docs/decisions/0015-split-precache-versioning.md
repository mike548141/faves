...
| `faves-data-<DATA_VERSION>` | `DATA_VERSION` | `data/index.json` + every `data/restaurants/<id>.json` | <!-- pathscan:allow: cache contents are runtime URLs relative to the site root, not repo paths — sw.js caches "data/index.json" -->
...
It stays in the shell cache. `data/index.json` reads like "configuration" (it <!-- pathscan:allow: runtime URL as the service worker names it, not a repo path -->
