...
(2026-09-08, ADR 0100). `app.css` sets `html { scroll-behavior: smooth }`, so a
...
GUESS** (owner-ruled on `340/200`, ADR 0100). A `untilPresent` timeout re-runs
...
— both deliberately excluded (ADR 0100).
...
timeouts only (above; ADR 0100). The reasoning is untouched, because a fresh
